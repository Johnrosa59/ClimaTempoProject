<h1>Projeto Nome: 📱Aplicativo Clima / Tempo</h1>
<h2>Finalidade</h2>
<p>Projeto desenvolvido para consumir dados da API e fazer um CRUD e os dados da API consumida é mostrado no RecyclerView.</p>
<p>O Layout teve ajuda da IA.</p>
<i>Video do projeto</i>



https://github.com/user-attachments/assets/fdd8fce7-08e9-4846-abb3-fa59fceeb6e8


<i>Imagem do projeto</i>
<img width="1918" height="1078" alt="image" src="https://github.com/user-attachments/assets/a1487d00-331a-4c70-a72c-5fb20e96410e" />







